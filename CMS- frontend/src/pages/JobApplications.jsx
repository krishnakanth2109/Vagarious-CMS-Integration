      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">{app.email} · {app.phone} · <span className="font-mono text-xs bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-bold uppercase">{app.seqId}</span></p>
          </div>
...
export default function JobApplications() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const appsWithIds = apps.slice().reverse().map((app, idx) => ({
        ...app,
        seqId: `VGS-${String(idx + 1).padStart(3, '0')}`
      })).reverse();
  ...
  const handleStatusChange = (updated) => {
    setApplications(prev => prev.map(a => (a._id === updated._id ? { ...updated, seqId: a.seqId } : a)));
  };
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-slate-50/70">
                  {['ID', 'Applicant', 'Applied For', 'Experience', 'Location', 'Skills', 'Status', 'Date', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {applications.map(app => (
                  <tr key={app._id} className="hover:bg-slate-50/60 transition-colors group">
                    {/* ID */}
                    <td className="px-5 py-4 whitespace-nowrap font-mono text-xs font-bold text-slate-400">
                      {app.seqId}
                    </td>
                    {/* Applicant */}
